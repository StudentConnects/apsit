...
function buildQuiz() {
  ...
  myQuestions.forEach((currentQuestion, questionNumber) => {
    ...
    for (letter in currentQuestion.answers) {
      ...
      answers.push(`<label class="container-checkbox">${letter} : ${
          currentQuestion.answers[letter]}
                        <input type="checkbox" name="question_${
...
function buildQuizNo() {
  ...
  myQuestions.forEach((questions, index) => {
    questionno.push(`<button class="btn btn-info btn-just-icon" value="${
...
function showIndexSlide(n) { showSlide(n); }
...
function quizTimer() {
  var sec = 1800, countDiv = document.getElementById("Quiztimer");
