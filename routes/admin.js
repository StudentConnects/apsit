...
router.get(
    ...
    (req, res) => {
        req.db
            .query(
                "Select id, name, description, logo from company where active = 1;")
...
router.post("/submitQuiz", (req, res) => {
  ...
  data.quiz_Info.quizName =
      data.quiz_Info.quizName.toLowerCase().split(" ").join("_");
  debug("MAKING ENTERING DB");
  req.db
      .query("call addQuiz1(?, ?, ?, ?);",
             [
               data.quiz_Info.quizName, data.quiz_Info.quizCompanyId,
               data.quiz_Info.quizTime, (data.quiz_Info.isActive) ? 1 : 0
             ])
      .then((results) => {
...
router.delete('/disableCompany', (req, res) => {
  const {id} = req.body;
  req.db
      .query('update company set active = 0 where id = ? and active= 1;',
             [ id ])
      .then(results => {
        // console.log(results);
        // res.send(results[0]);
...
router.patch('/enableCompany', (req, res) => {
  const {id} = req.body;
  req.db
      .query('update company set active = 1 where id = ? and active= 0;',
             [ id ])
      .then(results => {
        // console.log(results);
        // res.send(results[0]);
